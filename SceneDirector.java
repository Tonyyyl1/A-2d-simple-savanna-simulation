...
public class SceneDirector
{
                                              ...
                                              Field field)
    {
        ...
        Location safe = nearestSafeVisualCell(anchor.row, anchor.col,
            terrainMap, field, 12);
        if(safe == null) {
            safe = nearestPassableCell(anchor.row, anchor.col, terrainMap,
                field, 16);
        }
        if(safe == null) {
            return actor;
                                             ...
                                             Field field)
    {
        ...
        Location nearest = nearestSafeVisualCell(frame.row, frame.col,
            terrainMap, field, 8);
        if(nearest == null) {
            nearest = nearestPassableCell(frame.row, frame.col, terrainMap,
                field, 12);
        }
        if(nearest == null) {
                                                      ...
                                                      TerrainMap terrainMap)
    {
        ...
        double centerRow = row + 0.5;
        double centerCol = col + 0.5;
        double[][] samples = {
            {0.0, 0.0},
            {-0.34, 0.0},
            {0.34, 0.0},
        ...
        for(double[] sample : samples) {
            int sampleRow = clampCell((int)Math.floor(centerRow + sample[0]),
                                      terrainMap.getDepth());
            int sampleCol = clampCell((int)Math.floor(centerCol + sample[1]),
                                      terrainMap.getWidth());
