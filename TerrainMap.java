...
public class TerrainMap
{
    ...
    /**
     * True when the continuous grid coordinate lies on the painted water
     * surface. {@link #drawWaterhole} clips the water to the WATERHOLE cells
     * of {@code terrain[][]}, so the grid is the single source of truth for
     * both logic and visuals; this must stay a grid lookup, not an ellipse
     * approximation.
     */
    public boolean isVisuallyWaterAt(double row, double col)
    {
        int cellRow = clamp((int)Math.floor(row), 0, depth - 1);
        int cellCol = clamp((int)Math.floor(col), 0, width - 1);
        return terrain[cellRow][cellCol] == TerrainType.WATERHOLE;
    }
                               ...
                               int pixelHeight)
    {
        ...
        // Bright shoreline rim drawn on the region outline.
        g2.setColor(new Color(218, 203, 143, 170));
        g2.setStroke(new BasicStroke(Math.max(1.5f, pixelWidth * 0.004f),
